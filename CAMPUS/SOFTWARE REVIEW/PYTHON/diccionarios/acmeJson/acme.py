...
def updateDatabase(data,ruta):
    with open(ruta,'w',encoding='utf-8') as db:
        for i in data.keys():
            """ print(type(i)) """
            template = f'{i} : [{data[i]["nombre"]};{data[i]["hrsTra"]};{data[i]["vlrHra"]}]'
            db.write(str(template))
...
def menu():
    ...
    empleados = {}
    """ ruta = 'PYTHON/diccionarios/acme/database/db.txt' """
    ruta = 'acmeJson/database/db.json'
    with open(ruta) as fLDB:
        fJSON = json.loads(fLDB)
        print(fJSON)
        for linea in fJSON:
            linea = linea.split(';')
            empleados[int(linea[0])] = {}
            empleados[int(linea[0])]['nombre'] = linea[1]
            empleados[int(linea[0])]['hrsTra'] = linea[2]
            empleados[int(linea[0])]['vlrHra'] = linea[3].replace('\n','')
    ...
    while True:
        try:
            ...
            res = ingresoDatos(select,empleados,ruta)
